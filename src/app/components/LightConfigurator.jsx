...
import { FaChevronLeft, FaChevronRight, FaSave, FaShoppingCart, FaInfoCircle, FaQuestionCircle, FaSun, FaMoon, FaPlus, FaMinus } from "react-icons/fa";
    ...
    >
      ...
      <div className="relative">
        <motion.button
          ...
          className={`absolute left-0 top-0 bottom-0 flex items-center justify-center w-8 z-10 ${isDarkMode ? 'bg-gray-700/50' : 'bg-white/50'} ${
            scrollPosition <= 10 ? "opacity-50 cursor-not-allowed" : "opacity-100"
          }`}
          disabled={scrollPosition <= 10}
          whileHover={{ backgroundColor: isDarkMode ? 'rgba(55, 65, 81, 0.8)' : 'rgba(255, 255, 255, 0.8)' }}
          whileTap={{ scale: 0.95 }}
        >
          <FaChevronLeft className={isDarkMode ? 'text-white' : ''} />
        </motion.button>
        
        <div
          ref={scrollContainerRef}
          className="flex overflow-x-auto gap-4 py-2 px-8 cursor-grab active:cursor-grabbing select-none no-scrollbar"
        ...
        <motion.button
          ...
          className={`absolute right-0 top-0 bottom-0 flex items-center justify-center w-8 z-10 ${isDarkMode ? 'bg-gray-700/50' : 'bg-white/50'} ${
            scrollPosition >= maxScroll - 10 ? "opacity-50 cursor-not-allowed" : "opacity-100"
          }`}
          disabled={scrollPosition >= maxScroll - 10}
          whileHover={{ backgroundColor: isDarkMode ? 'rgba(55, 65, 81, 0.8)' : 'rgba(255, 255, 255, 0.8)' }}
          whileTap={{ scale: 0.95 }}
        >
          <FaChevronRight className={isDarkMode ? 'text-white' : ''} />
        </motion.button>
        
        {/* Gradient overlays for horizontal scroll indication */}
        <div className="absolute top-0 right-0 w-12 h-full bg-gradient-to-l from-[#232B2B] to-transparent pointer-events-none"></div>
        <div className="absolute top-0 left-0 w-12 h-full bg-gradient-to-r from-[#232B2B] to-transparent pointer-events-none"></div>
        
        {/* Removed custom drag cursor */}
      </div>
    </motion.div>
  );
};
...
const PendantConfigurator = ({ pendants, updatePendantDesign, isDarkMode }) => {
  return (
    <motion.div 
      ...
      transition={{ duration: 0.5 }}
    >
      <h3 className="text-lg font-bold mb-4 font-['Amenti']">Individual Pendant Configuration</h3>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center space-x-1">
                <div 
                  ...
                  style={{ backgroundColor: pendant?.color || '#50C878' }}
                ></div>
                <span className="text-sm">{pendant?.design || 'bumble'}</span>
              </div>
            </div>
...
const LightConfigurator = () => {
  ...
  const handleCableChange = (color, length) => {
    setCableColor(color);
    setCableLength(length);
    toast.info(`Cable changed to ${color}, ${length}`, {
      position: "bottom-right",
      autoClose: 1500,
      theme: isDarkMode ? "dark" : "light"
    });
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          >
            ...
            >
              <div className="absolute inset-0 flex items-center justify-center text-gray-400">
                <div className="text-center">
                  <p className={`text-xl mb-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>Preview Area</p>
                  <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>(iframe will be embedded here)</p>
                  
                  {/* Placeholder visualization based on current configuration */}
            ...
            <motion.div 
              className={`mt-6 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.3 }}
            >
              <div className="flex flex-col mb-4">
                ...
                <div className={`mt-3 p-3 rounded-lg text-sm ${isDarkMode ? 'bg-gray-700/50 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>
          ...
          <motion.div 
            className={`lg:w-2/5 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6 h-fit`}
            initial={{ opacity: 0, x: 30 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: 0.2 }}
          >
        ...
        {/* Additional Information Section - Moved outside the configuration panel */}
