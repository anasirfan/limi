...
const InteractiveStory = () => {
  ...
  useEffect(() => {
    ...
    ScrollTrigger.getAll().forEach(st => {
      if (st.vars.trigger === containerRef.current || 
          (st.vars.trigger && st.vars.trigger.classList && 
    ...
    let ctx = gsap.context(() => {
      ...
      timeline.addLabel('formTriangle', 'showComponents+=1.6');
      ...
      timeline.addLabel('showImages', 'formTriangle+=1.2')
        .to('.div-name', {
          opacity: 0,
          duration: 0.5,
        }, 'showImages')
        .to(hubRef.current.querySelector('.div-image'), {
          opacity: 1,
          duration: 0.5,
        }, 'showImages+=0.5')
        .to(baseRef.current.querySelector('.div-image'), {
          opacity: 1,
          duration: 0.5,
        }, 'showImages+=0.7')
        .to(appRef.current.querySelector('.div-image'), {
          opacity: 1,
          duration: 0.5,
        }, 'showImages+=0.9');

      // PHASE 5: Show and animate triangle lines
      timeline.addLabel('showLines', 'showImages+=1.2')
        .to(svgRef.current.querySelectorAll('.main-triangle-line'), {
          opacity: 1,
          duration: 0.5,
        }, 'showLines')
        .fromTo('.main-triangle-line', {
      ...
      timeline.addLabel('showDescriptions', 'showLines+=1.5')
        .to('.description-line', {
          opacity: 1,
          duration: 0.5,
          stagger: 0.2,
  ...
  return (
    <div 
      ref={containerRef} 
      className={`relative ${isMobile ? 'h-auto py-16' : 'h-screen'} w-full overflow-hidden bg-[#292929]`}
    >
      {/* Heading */}
      <h1
        ref={headingRef}
      ...
      {!isMobile && (
        <div className="triangle-container relative h-full w-full">
          ...
          <div
            ref={hubRef}
            className={`${isMobile ? 'relative mx-auto mb-4' : 'absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform'} h-20 w-20 sm:h-28 sm:w-28 md:h-36 md:w-36 lg:h-40 lg:w-40 rounded-lg bg-[#176D63] p-4 z-20`}
          >
            <div className="div-name text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-white text-center mb-1 sm:mb-2">Hub</div>
            <div className={`div-image ${isMobile ? 'opacity-100' : 'opacity-0'} absolute inset-0`}>
          ...
          <div
            ref={baseRef}
            className={`${isMobile ? 'relative mx-auto mb-4' : 'absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform'} h-20 w-20 sm:h-28 sm:w-28 md:h-36 md:w-36 lg:h-40 lg:w-40 rounded-lg bg-[#0E5A3D] p-4 z-20`}
          >
          ...
          <div
            ref={appRef}
            className={`${isMobile ? 'relative mx-auto mb-4' : 'absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform'} h-20 w-20 sm:h-28 sm:w-28 md:h-36 md:w-36 lg:h-40 lg:w-40 rounded-lg bg-[#2E1F3E] p-4 z-20`}
          >
