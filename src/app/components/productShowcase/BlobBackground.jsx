...
// BlobBackground component accepts color and seed props for customization
const BlobBackground = ({ color = '#50C878', seed = 1 }) => {
  ...
  // Initialize blobs with deterministic values once on mount
  useEffect(() => {
    if (!isClient) return;
    ...
    const svgBlobs = [
      {
        ...
        fill: color.replace(')', ', 0.3)').replace('rgb', 'rgba'),
      ...
      },
      {
        ...
        transform: 'translate(100 100)',
        fill: color.replace(')', ', 0.25)').replace('rgb', 'rgba'),
        preserveAspectRatio: 'none'
      },
      {
        id: 'blob3',
      ...
      },
      {
        ...
        path: 'M39.6,-67.5C51.4,-61.7,61.1,-51.3,67.7,-39.3C74.2,-27.3,77.6,-13.6,78.3,0.4C79.1,14.5,77.2,29,70.8,41.2C64.3,53.4,53.4,63.4,40.8,71.8C28.2,80.2,14.1,87,-0.8,88.3C-15.7,89.6,-31.3,85.5,-44.9,77.7C-58.6,70,-70.2,58.6,-77.8,45C-85.4,31.5,-89,15.7,-86.4,1.5C-83.9,-12.8,-75.2,-25.6,-67.6,-39.2C-60.1,-52.9,-53.6,-67.3,-42.6,-73.5C-31.5,-79.8,-15.7,-77.8,-0.9,-76.1C13.9,-74.5,27.7,-73.3,39.6,-67.5Z',
        transform: 'translate(100 100)',
        fill: color.replace(')', ', 0.12)').replace('rgb', 'rgba'),
        preserveAspectRatio: 'none'
      }
    ];
    ...
    const createBlobs = () => {
      // Use fixed dimensions for server-side rendering to avoid hydration errors
      // We'll update positions on client side after hydration
      ...
      for (let i = 0; i < 5; i++) {
        ...
        // Use fixed values for initial render to avoid hydration errors
        const xPos = Math.round(position.x * width);
        const yPos = Math.round(position.y * height);
        ...
        // Use deterministic values for animation properties
        // This ensures server and client render the same values
        const seedOffset = i * 100;
        const animX = Math.round(40 + random(0, 30));
        const animY = Math.round(40 + random(0, 30));
        const animDuration = Math.round(30 + random(0, 15));
        ...
        newBlobs.push({
          ...
          size,
          opacity: baseOpacity,
          rotation: Math.round(random(0, 360)), // Deterministic rotation
          animationX: animX,
          animationY: animY,
    ...
    // Initialize blobs once with deterministic values
    createBlobs();
    
    // Only update positions on client after hydration
    if (isClient) {
      // Update blob positions based on actual window size
      const updateBlobPositions = () => {
        const width = window.innerWidth;
        const height = window.innerHeight;
        
        blobsRef.current = blobsRef.current.map((blob, i) => {
          const positionPatterns = [
            { x: 0.5, y: 0.5 },
            { x: 0.1, y: 0.1 },
            { x: 0.9, y: 0.1 },
          ...
          const position = positionPatterns[i % positionPatterns.length];
          return {
            ...blob,
  ...
  return (
    <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden">
      {isClient && blobsRef.current.map((blob) => (
        <motion.div
          key={blob.id}
          className="absolute"
          style={{
